        ...
        if (tbody) {
          tbody.innerHTML = json.data.map(c => `
            <tr>
              ...
              <td>${c.slug}</td>
              <td>${c.description || '-'}</td>
              <td><span style="font-size:0.8rem; color:var(--admin-text-muted);">Active</span></td>
            </tr>
          `).join('');
        }
      }
          ...
          grid.innerHTML = json.data.map(m => `
            <div class="media-item-card">
              <div class="media-img-box">
                <img src="${m.file_path}" alt="${m.alt_text || m.file_name}">
              </div>
              <div class="media-item-info">
                <div class="media-item-title">${m.file_name}</div>
